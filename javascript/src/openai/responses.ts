...
export class OpenAIResponsesClient {
  ...
  private buildResponsesParams(
    ...
    return {
      ...
      // Build include array with reasoning.encrypted_content when reasoning is requested
      ...((responsesOptions?.include ||
        input.reasoning ||
        responsesOptions?.reasoning) && {
        include: [
          ...(responsesOptions?.include || []),
          // Automatically include reasoning.encrypted_content when reasoning is requested
          ...((input.reasoning || responsesOptions?.reasoning) &&
          !responsesOptions?.include?.includes("reasoning.encrypted_content")
          ...
          for (const part of message.content) {
            switch (part.type) {
              ...
              case "audio":
                ...
                break;
              case "reasoning":
                // Include reasoning parts with signature for model recognition
                if (part.signature) {
                  content.push({
                    type: "input_text",
                    text: `[Reasoning with signature: ${part.signature}]`,
                  });
                }
                break;
            }
  ...
  private mapEventToContentDeltas(
    ...
    switch (event.type) {
      ...
      case "response.reasoning.done": {
        // When reasoning is done, we get the complete text
        // The signature should be available in the final response
        const part: ReasoningPartDelta = {
          type: "reasoning",
